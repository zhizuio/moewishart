...
[![CRAN](http://www.r-pkg.org/badges/version/moewishart)](https://cran.r-project.org/package=moewishart)
...
four different inference algorithms for the two model:
...
- Mixture-of-Expert model, in which the gating probabilities depend on
  covariates:
...
[CRAN](https://CRAN.R-project.org/package=moewishart):
...
[GitHub](https://github.com/zhizuio/moewishart):
...
- $q=3$ gating covariates
  $\mathbf X = [x_{ij}] \in \mathbb R^{n\times q}$,
  $x_{ij}\sim\text{N}(0,1)$, $i=1,...,n$, $j=1,...,q$
- Fixed covariate effects
  $\boldsymbol\beta=[\boldsymbol\beta_1,...,\boldsymbol\beta_K] \in \mathbb R^{q\times K}$,
  with $\boldsymbol\beta_{K}=0$
- Probabilities of subpopulations
  $\boldsymbol\pi = [\pi_{ik}] \in \mathbb R^{n\times q}$,
  $\pi_{ik} = \exp(\mathbf X_i\boldsymbol\beta_k) / \sum_{l=1}^K\exp(\mathbf X_i\boldsymbol\beta_l)$,
  $k=1,...,K$
- Degrees of freedom $\boldsymbol\nu = (\nu_1,\nu_2,\nu_3) = (8, 12, 3)$
- Scale matrices of the Wishart distribution $\Sigma_1$, $\Sigma_2$,
  $\Sigma_3 \in \mathbb R^{p\times p}$
- Data
  $S_i \sim \pi_{i1}\text{Wishart}(\nu_1, \Sigma_1) + \pi_{i2}\text{Wishart}(\nu_2, \Sigma_2) + \pi_{i3}\text{Wishart}(\nu_3, \Sigma_3)$
...
<br> Posterior means for scale matrices of Wishart distributions:
...
<br> Posterior means for gating coefficients:
...
<br> Posterior means for subpopulation probabilities:
...
<br> EM estimates for Wishart scale matrices:
...
<br> EM estimates for gating coefficients:
...
<br> EM estimates for DoF of Wishart distributions:
...
<br> EM estimate for Wishart scale matrices:
...
Mai TT, Zhao Z (2026). Mixture-of-experts Wishart model for covariance
matrices with an application to Cancer drug screening.
[arXiv:2602.13888](https://doi.org/10.48550/arXiv.2602.13888).
