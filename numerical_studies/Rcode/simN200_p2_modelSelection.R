...
file_directory <- "/Users/zhiz/Downloads/moewishart/"
setwd(file_directory)

# summarize information criteria
datIC <- data.frame(model = NULL, K = NULL, simIdx = NULL, elpd = NULL, icl = NULL, aic = NULL, bic = NULL)
...
for (my_K_intial in 2:6) {
  ...
  filename_save <- paste0(file_directory, "n200_p", p, "K", my_K_intial, "_sim")
  ...
  for (seed.idx in 1:n_sim) {
    ...
    datIC <- rbind(datIC, data.frame(model = "Bayes", K = my_K_intial, simIdx = seed.idx, elpd = elpd, icl = icl, aic = NA, bic = NA))
    ...
    datIC <- rbind(datIC, data.frame(model = "Bayes-MoE", K = my_K_intial, simIdx = seed.idx, elpd = elpd, icl = icl, aic = NA, bic = NA))
    ...
    datIC <- rbind(datIC, data.frame(model = "EM", K = my_K_intial, simIdx = seed.idx, elpd = NA, icl = icl, aic = aic, bic = aic))
    ...
    datIC <- rbind(datIC, data.frame(model = "EM-MoE", K = my_K_intial, simIdx = seed.idx, elpd = NA, icl = icl, aic = aic, bic = aic))

  }
}


IC_mean <- data.frame("model" = NULL, K = NULL, elpd = NULL, icl = NULL, aic = NULL, bic = NULL)
IC_sd <- data.frame("model" = NULL, K = NULL, elpd = NULL, icl = NULL, aic = NULL, bic = NULL)
for (model in unique(datIC$model)){
  for (k in 2:6) {
    tmp <- datIC[datIC$model == model & datIC$K == k, 4:7]
...
IC_mean; rownames(IC_mean) <- NULL
IC_sd; rownames(IC_mean) <- NULL

IC_mean_sd <- IC_mean
IC_mean_sd[, 3:6] <- matrix(paste0(as.matrix(round(IC_mean[, 3:6],1)), " (", as.matrix(round(IC_sd[, 3:6],1)), ")"), nrow=nrow(IC_mean))
